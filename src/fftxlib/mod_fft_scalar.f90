...
#define  FFTW_ESTIMATE 64
        ...
        INTEGER   :: cft_b_bw_dims( 4 )
        C_POINTER :: cft_b_bw_planz = 0
        C_POINTER :: cft_b_bw_planx = 0
        C_POINTER :: cft_b_bw_plany = 0
        !
        INTEGER   :: cft_b_fw_dims( 4 )
        C_POINTER :: cft_b_fw_planz = 0
        C_POINTER :: cft_b_fw_planx = 0
        C_POINTER :: cft_b_fw_plany = 0
...
!     parameters nz, nsl, ldz) are stored and re-used if available
     ...
     !   C_POINTER is defined in fftqe_defs.h
...
!$omp do
       DO i=1, nsl
          ...
          !
          cout( offset : offset+ldz_t-1 ) = c( offset : offset+ldz_t-1 ) 
       END DO
!$omp end do
!$omp end parallel
...
!
   ...
   SUBROUTINE cft_b_omp_init ( nx, ny, nz, i_sign )
...
!$omp single
      ...
      IF ( i_sign == 1 ) THEN
          ...
          END IF
          !
      ELSE IF ( i_sign == -1 ) THEN 
          !
          IF( cft_b_fw_planz  == 0 ) THEN
             CALL CREATE_PLAN_1D( cft_b_fw_planz, nz, -1 )
             cft_b_fw_dims(3) = nz
          END IF
          IF( cft_b_fw_planx  == 0 ) THEN
             CALL CREATE_PLAN_1D( cft_b_fw_planx, nx, -1 )
             cft_b_fw_dims(1) = nx
          END IF
          IF( cft_b_fw_plany  == 0 ) THEN
             CALL CREATE_PLAN_1D( cft_b_fw_plany, ny, -1 )
             cft_b_fw_dims(2) = ny
          END IF
          !
      ENDIF
...
!$omp threadprivate (k)
      ...
      IF ( sgn == 1 ) THEN
          ...
          IF ( ( cft_b_bw_planz == 0 ) .or. ( cft_b_bw_planx == 0 ) .or. ( cft_b_bw_plany == 0 ) ) THEN
             CALL errore('cft_b_omp','bw plan not initialized',1)
          END IF
          !
          IF ( ( nx /= cft_b_bw_dims(1) ) .or. ( ny /= cft_b_bw_dims(2) ) .or. ( nz /= cft_b_bw_dims(3) ) ) THEN
             CALL errore('cft_b_omp', 'dimensions are inconsistent with the existing bw_plan',1) 
          END IF
          !
      ELSEIF ( sgn == -1 ) THEN
          !
          IF ( ( cft_b_fw_planz == 0 ) .or. ( cft_b_fw_planx == 0 ) .or. ( cft_b_fw_plany == 0 ) ) THEN
             CALL errore('cft_b_omp','fw plan not initialized',1)
          END IF
          !
          IF ( ( nx /= cft_b_fw_dims(1) ) .or. ( ny /= cft_b_fw_dims(2) ) .or. ( nz /= cft_b_fw_dims(3) ) ) THEN
             CALL errore('cft_b_omp', 'dimensions are inconsistent with the existing fw_plan',1) 
          END IF
          !
      ENDIF
      ...
      ELSE IF ( sgn == -1) THEN
          call FFTW_INPLACE_DRV_1D( cft_b_fw_planz, ldx*ldy, f(1), ldx*ldy, 1 )
      ENDIF
      !
      !  fft along Y
      !  fft along X
      ...
      ELSE IF ( sgn == -1 ) THEN
          !
          do k = imin3, imax3
             call FFTW_INPLACE_DRV_1D( cft_b_fw_plany, nx, f((k-1)*ldx*ldy + 1), ldx, 1 )
             call FFTW_INPLACE_DRV_1D( cft_b_fw_planx, ny, f((k-1)*ldx*ldy + 1), 1, ldx )
          end do   
          !
      ENDIF
