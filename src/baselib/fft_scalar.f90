...
#if undef __FFTMKL8
     !
     ! just to avoid makedeps.sh to complain
     !
     MODULE MKL_DFT_TYPE
     END MODULE
#endif
...
!     parameters nz, nsl, ldz) are stored and re-used if available
     ...
     !   C_POINTER is defined in include/ffth_defs.h
...
!     parameters nx,ny,nzl,ldx) are stored and re-used if available
     ...
     REAL (dbl), SAVE :: fw_tablex( ltabl, ndims ), fw_tabley( ltabl, ndims )
     REAL (dbl), SAVE :: bw_tablex( ltabl, ndims ), bw_tabley( ltabl, ndims )

#elif defined __SCSL

     INTEGER, PARAMETER :: ltabl = 2 * nfftx + 256
     REAL (dbl), SAVE :: tablex (ltabl, ndims), tabley(ltabl, ndims)
     ...
     REAL (dbl), SAVE :: tablex (ltabl, ndims)
     REAL (dbl), SAVE :: tabley (ltabl, ndims)
...
!     parameters nx,ny,nz) are stored and re-used if available
     ...
     REAL (dbl), SAVE :: table (ltabl, ndims)
     REAL (dbl)       :: DUMMY
     INTEGER, SAVE :: isys(0:1) = (/ 1, 1 /)
     ...
     REAL (dbl), SAVE :: auxp (lwork, ndims)
     ! not sure whether auxp is work space or not
     COMPLEX(dbl), DIMENSION(:), ALLOCATABLE :: cw2   
...
SUBROUTINE cfft3ds (f, nx, ny, nz, ldx, ldy, ldz, isign, &
  ...
  INTEGER, PARAMETER :: ltabl = 20000 + 3 * nfftx
  REAL (dbl), SAVE :: fw_table( ltabl, 3, ndims )
  REAL (dbl), SAVE :: bw_table( ltabl, 3, ndims )
