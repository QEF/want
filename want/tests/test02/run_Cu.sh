...
if [ "$SCF" = ".TRUE." ] ; then  
   ...
   $PARA_PREFIX  $DFT_BIN/pw.x $PARA_POSTFIX < $TEST_HOME/scf_Cu.in > $TEST_HOME/scf_Cu.out
   if [ $? = 0 ] ; then 
      echo "$ECHO_T done" 
   else
...
if [ "$NSCF" = ".TRUE." ] ; then  
   ...
   $PARA_PREFIX  $DFT_BIN/pw.x $PARA_POSTFIX < $TEST_HOME/nscf_Cu.in > $TEST_HOME/nscf_Cu.out
   if [ $? = 0 ] ; then 
      echo "$ECHO_T done" 
   else
      echo "$ECHO_T problems found" ; exit 1
...
if [ "$PWEXPORT" = ".TRUE." ] ; then  
   ...
   $PARA_PREFIX  $DFT_BIN/pw_export.x $PARA_POSTFIX  \
              <  $TEST_HOME/pwexport_Cu.in > $TEST_HOME/pwexport_Cu.out
   if [ $? = 0 ] ; then 
      echo "done" 
   else
...
if [ "$DFT_BANDS" = ".TRUE." ] ; then  
   ...
   $PARA_PREFIX  $DFT_BIN/pw.x $PARA_POSTFIX \
               < $TEST_HOME/dft_bands_Cu.in > $TEST_HOME/dft_bands_Cu.out
   if [ $? = 0 ] ; then 
      echo "$ECHO_T done" 
   else
...
if [ "$DISENTANGLE" = ".TRUE." ] ; then  
   ...
   $WANT_BIN/disentangle.x < $TEST_HOME/want_Cu.in > $TEST_HOME/disentangle_Cu.out
   if [ ! -e CRASH ] ; then 
      echo "$ECHO_T done" 
   else
      echo "$ECHO_T problems found" ; cat CRASH ; exit 1
...
if [ "$WANNIER" = ".TRUE." ] ; then  
   ...
   $WANT_BIN/wannier.x < $TEST_HOME/want_Cu.in > $TEST_HOME/wannier_Cu.out
   if [ ! -e CRASH ] ; then 
      echo "$ECHO_T done" 
   else
      echo "$ECHO_T problems found" ; cat CRASH ; exit 1
...
if [ "$BANDS" = ".TRUE." ] ; then  
   ...
   $WANT_BIN/bands.x < $TEST_HOME/bands_Cu.in > $TEST_HOME/bands_Cu.out
   if [ ! -e CRASH ] ; then 
      echo "$ECHO_T done" 
   else
      echo "$ECHO_T problems found" ; cat CRASH ; exit 1
...
if [ "$PLOT" = ".TRUE." ] ; then
   ...
   $WANT_BIN/plot.x < $TEST_HOME/plot_Cu.in > $TEST_HOME/plot_Cu.out
   if [ ! -e CRASH ] ; then
      echo "$ECHO_T done"
   else
      echo "$ECHO_T problems found" ; cat CRASH ; exit 1
...
if [ "$CHECK" = ".TRUE." ] ; then
   ...
   #
   cd $TEST_HOME
   list="disentangle_Cu.out wannier_Cu.out "
   #
   for file in $list
   do
