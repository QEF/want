...
! ...   declare modules
        ...
        USE parser_module,         ONLY : version_compare, matches
        USE iotk_module
        !
        PRIVATE
        PUBLIC :: read_upf_v2
...
SUBROUTINE read_upf_v2(u, upf, grid, ierr)             !
   ...
   !
   USE pseudo_types_module, ONLY: nullify_pseudo_upf, deallocate_pseudo_upf
   USE radial_grids_module, ONLY: radial_grid_type, nullify_radial_grid
   IMPLICIT NONE
...
!
   SUBROUTINE read_gipaw(u, upf)
      ...
      IF(upf%gipaw_data_format /= 2) &
         CALL warning('read_upf_v2::read_gipaw','Unknown format version')
      !
