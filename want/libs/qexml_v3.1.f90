...
!
    ...
    SUBROUTINE qexml_read_gk( ik, npwk, npwkx, index, igk, ierr )
      ...
      IF ( PRESENT( npwkx )  ) THEN
          ...
          IF (ierr/=0) RETURN
          !
          CALL iotk_scan_dat( iunpun, "MAX_NPW", npwkx, IERR=ierr )
          IF (ierr/=0) RETURN
          !
          CALL iotk_scan_end( iunpun, "PLANE_WAVES", IERR=ierr )
          IF (ierr/=0) RETURN
      ...
      IF (ierr/=0) RETURN
      !
      IF ( PRESENT( npwk ) .OR. PRESENT( index ) .OR. PRESENT( igk ) )  THEN
          !
          CALL iotk_scan_begin( iunpun, "INDEX", ATTR=attr , IERR=ierr )
          IF (ierr/=0) RETURN
          !
          CALL iotk_scan_attr( attr, "size", npwk_ , IERR=ierr )
          IF (ierr/=0) RETURN
          !
          CALL iotk_scan_end( iunpun, "INDEX", IERR=ierr )
          IF (ierr/=0) RETURN
          !
      ENDIF
      !
    ...
    SUBROUTINE qexml_read_bands( nbnd, num_k_points, nspin, natomwfc, ef, nelec, &
      ...
      !
      !
      ! first read kpt num from a different section
      !
      CALL iotk_scan_begin( iunpun, "BRILLOUIN_ZONE", IERR=ierr )
      IF (ierr/=0) RETURN
      !
    ...
    SUBROUTINE qexml_read_wfc( ibnds, ibnde, ik, ispin, npwk, igk, ngw, igwx, wf, wf_kindip, ierr )
      ...
      IF (ierr/=0) RETURN
      !
      CALL iotk_scan_begin( iunpun, "WFC", IERR=ierr )
      IF (ierr/=0) RETURN
      !
      !
      ! to be fixed in the fmt
      !
      IF ( ispin == 2 ) THEN
          !
          CALL iotk_scan_end( iunpun, "WFC", IERR=ierr )
          IF (ierr/=0) RETURN
          !
          CALL iotk_scan_begin( iunpun, "WFC", IERR=ierr )
          IF (ierr/=0) RETURN
